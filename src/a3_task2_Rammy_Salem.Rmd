...
```
### In Row 49, the name after the second dash seems to have been omitted.
...
sw_skin_sep <- starwars %>% # Start from starwars data
  separate(col = skin_color, # Separate the `name` column
...
mpg_model_class <- mpg %>% # Start with mpg
  unite(col = model_class, # Create a new column called 'model_class'
        c(model,class), # That combines the model and class columns
        sep = '-', # Places a dash between the pieces
        remove = FALSE) # And do not remove the original column
...
gasol_hookshots <- lakers %>% # Start with lakers dataset
  filter(player == "Pau Gasol", type == "hook") %>% # filter to only include Pau Gasol's hook shots
  mutate(date_new = ymd(date)) %>% # Added a new date column with "Date" class
  mutate(month = month(date_new)) %>%  # Added a new column containing the month, parsed from date_new
  count(month) # counts number of Gasol's hook shots by month for the 2008-2009 season
...
storms_danger <- storms %>% # Start from storms data
  ...
  status == "hurricane" ~ "danger", # "danger" if status is "hurricane"
  status != "hurricane" ~ "moderate" # "moderate" if status is not "hurricane"
  )
  )
```
